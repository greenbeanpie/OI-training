...
try:
    if problem_name[0] == "P":
        ...
        os.chdir(work_dir+"Codeforce\\"+sub_dir+"\\"+problem_name[len(problem_name)-1:])
...
timeout*=3.5 # Don't ask me why. I just know the computer on Luogu platform is 3.5x faster than the computer in my school. You can change this in your own way.
...
def check(cnt1):
    ...
    for i in range(1, cnt1 + 1):
        ...
        if generator_cpp:
            # os.system(".\\generator")
            stdin,stderr = subprocess.Popen([".\\generator.exe"],stdin = subprocess.PIPE,stdout=subprocess.PIPE).communicate()
        else:
            # os.system("python generator.py")
            stdin,stderr = subprocess.Popen(["python","generator.py"],stdin = subprocess.PIPE,stdout=subprocess.PIPE).communicate()
        if validator:
            while subprocess.Popen([".\\validator.exe"]).returncode != 0:
                stdin,stderr = subprocess.Popen(["python","generator.py"],stdin = subprocess.PIPE,stdout=subprocess.PIPE).communicate()
        # launch your program here
        start_time = time.perf_counter()
        prcTJ = subprocess.Popen([".\\" + problem_name + "_TJ"],stdin = subprocess.PIPE,stdout=subprocess.PIPE)
        output_tj,err_TJ = prcTJ.communicate(input=stdin)
        output_hmz=""
        proc = subprocess.Popen([".\\" + problem_name],stdin = subprocess.PIPE,stdout=subprocess.PIPE)
        try:
            ...
            lock1.acquire()
            tot=tot+1
            result.add_row([str(tot), "\033[35m Time Limit Exceeded \033[0m", ">1000ms"])
            lock1.release()
            if stopwhennotac:
                    break
            continue
        ...
        while output_hmz[len(output_hmz)-1:len(output_hmz)]=='\n' or output_hmz[len(output_hmz)-1:len(output_hmz)]==' ':
            output_hmz = output_hmz[:len(output_hmz)-1]
        while output_tj[len(output_tj)-1:len(output_tj)]=='\n' or output_tj[len(output_tj)-1:len(output_tj)]==' ':
            output_tj = output_tj[:len(output_tj)-1]
        lock1.acquire()
        tot=tot+1
        if output_tj != output_hmz:
            result.add_row(
                [str(tot), "\033[31m Wrong Answer \033[0m", str(round(used_time*1000)) + "ms"]
            )
            if stopwhennotac:
                break
        else:
            result.add_row(
                [str(tot), "\033[32m Accepted \033[0m", str(round(used_time*1000)) + "ms"]
            )
        lock1.release()
    ...
    for tot in range(1, cnt + 1):
        ...
        if not (spj):
            ...
            try:
                ...
                result.add_row([str(tot), "\033[35m Time Limit Exceeded \033[0m", ">1000s"])
                if stopwhennotac:
                    break
                # result.add_row([str(tot), "\033[35m Runtime Error \033[0m", "-1ms"])
                continue
            ...
            if res.read().find("Wrong Answer"):
                result.add_row(
                    [str(tot), "\033[31m Wrong Answer \033[0m", str(round(used_time/3.5)) + "ms"]
                )
                if stopwhennotac:
                    break
            else:
                result.add_row(
                    [str(tot), "\033[32m Accepted \033[0m", str(round(used_time/3.5)) + "ms"]
                )
        print("checking in progress: " + str(tot) + "/" + str(cnt), end="\r")
