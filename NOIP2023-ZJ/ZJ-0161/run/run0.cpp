...
//		printf("check: %d %d\n",Now_L,Now_R);
		build(1,Now_L,Now_R);
		Sum[Now_L-1]=Sum[Now_R+1]=0;P[Now_L-1]=P[Now_R+1]=INF;
		for(int i=Now_L;i<=Now_R;i++) Sum[i]=Sum[i-1]+W[i],P[i]=INF;
		...
		for(int i=Now_L;i<=Now_R;i++) P[i]=min(P[i],P[i-1]);
...
//		for(int i=Now_L;i<=Now_R;i++) printf("%d ",P[i]);puts("");
		...
		update(1,Now_L,Now_R,Now_L,F[Now_L]-Sum[R_Min[Now_L]-1]+1ll*D*(L[R_Min[Now_L]]-1));
		for(int i=Now_L+1;i<=Now_R;i++){
			F[i]=Sum[i]+query(1,Now_L,Now_R,P[L_Min[i]],i-1)-1ll*D*R[i];
			F[i]=max(F[i],W[i]-1ll*(R[i]-L[i]+1)*D);
			F[i]=max(F[i],F[i-1]);
			update(1,Now_L,Now_R,i,F[i]-Sum[R_Min[i]-1]+1ll*D*(L[R_Min[i]]-1));
		}
