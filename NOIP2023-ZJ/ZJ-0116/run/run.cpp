...
int main(){
    ...
    while(T--){
        ...
        if(19<=testId&&testId<=21){
            _.build(1,1,m);
            long long sum=0;
            int now=1,now2=0;
            for(int i=1;i<=m;i++){
                while(now2<m&&a[now2+1].x+1<=a[i].x-a[i].y)now2++;
                _.updateAddSingle(1,i,dp[now2]+1ll*(a[i].x-a[i].y)*d-sum);
                // printf("%d %lld %lld %lld\n",i,(i==1?0:dp[i-2]),1ll*(a[i].x-a[i].y)*d,-sum);
                dp[i]=dp[i-1],sum+=a[i].v;
                int l=a[i].x-a[i].y+1,r=a[i].x;
                while(now<=m&&r-(a[now].x-a[now].y)>k)now++;
                // printf("%d %d %d\n",i,now,now2);
                if(now<=i){
                    // printf("%lld %lld %d\n",_.queryMax(1,now,i),-1ll*r*d,sum);
                    dp[i]=max(dp[i],_.queryMax(1,now,i)-1ll*r*d+sum);
                }
                // printf("dp[%d] = %lld\n",i,dp[i]);
            }
            printf("%lld\n",dp[m]);
