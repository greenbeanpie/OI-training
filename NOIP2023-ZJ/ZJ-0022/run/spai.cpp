...
struct opt{
    ...
    void calc(){
        ...
        return;*/
        cll l_=std::lower_bound(x+1,x+tx+1,l-1)-x+1,
        r_=std::lower_bound(x+1,x+tx+1,r)-x;
        if(op==1)dt.update(l_,r_,c);
        else{
        *res=dt.query(l_,r_)+c;
            //printf("res=%lld %lld %lld\n",dt.query(l_,r_),c,*res);
            }
    }
}q[N];
aN f;
...
void calc(cll ID){
    (void)ID;
    ull wuh=rd();
    //wuh=13109570281517897720ull;
   // wuh=17462938647148434322ull;
    //wuh=2583272014892537200;
    std::mt19937_64 rd(wuh);
    printf("id=%lld wuh=%llu\n",ID,wuh);
    //cll n=readll(),m=readll(),k=readll(),d=readll();
    ll n=rd()%(ll)1e3+1,m=rd()%100000,k=rd()%n+1,d=rd()%5000;
    //printf("%lld %lld %lld %lld\n",n,m,k,d);
    rep(i,1,m){
        cll l=rd()%n+1,r=rd()%n+1,c=rd()%50000000;
        p[i]=(node){std::min(l,r),std::max(l,r),c};
        //printf("%lld %lld %lld\n",l,r,c);
    }
    //rep(i,1,m)p[i].read();
    //if(n<=1000){
    ...
    //}else{
        ...
        dt.set(tx,d);
        //rep(i,1,tx)printf("%lld\n",x[i]);
        
        //rep(i,1,n+1)g[i]=i*d;
        
        rep(i,1,tq-1)/*printf("%lld %lld %lld %lld\n",q[i].l,q[i].r,q[i].op,q[i].c),*/q[i].calc();
       // printf("%lld\n",q[tq].c);
        rep(i,1,tq)if(q[i].op==0)/*printf("%lld %lld %lld\n",*q[i].res,f[std::min(n+1,q[i].r)],q[i].r),*/assert(*q[i].res==f[std::min(n+1,q[i].r)]);
    //}
}
int main(){
    rep(i,1,T)calc(i);
