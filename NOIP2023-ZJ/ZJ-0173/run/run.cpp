...
//		if(19<=C&&C<=21){
		if(C>=15){
			...
			for(int i=1;i<=m;i++){
				...
				{
//				if(a[1].y-a[1].x+1<=o){
					ans=max(ans,a[1].z-1ll*(a[1].y-a[1].x+1)*d);
				}
			}
			cout<<ans<<'\n';
			continue;
...
//			b.push_back(a[i].y+o);
		...
		for(int i=0;i<=n;i++)l[i].clear(),xd[i].clear(),ys[i].clear();
		for(int i=1;i<=m;i++){
			l[a[i].x].push_back(i);
		}
		for(int i=1;i<=n;i++){
			for(int j=1;j<=n;j++)ys[j].clear();
			loli sum=0;
			for(int j=i;j<=n;j++){
				if(j-i+1>o)break;
//				cout<<"solve "<<b[i]<<' '<<b[j]<<'\n';
				for(int k:l[j])ys[a[k].y].push_back(a[k].z);
...
//				xd.push_back(zzz{i+1,j+1,sum});
				if(sum>0)xd[j].emplace_back(i,sum);
//				cout<<b[i]<<' '<<b[j]<<' '<<sum<<'\n';
