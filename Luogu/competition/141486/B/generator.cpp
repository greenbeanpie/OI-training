...
namespace Main{
	...
	uniform_int_distribution<> dist(l,r),dist2(1,1e7);
	int main(){
		int m = 20, q = 3e5, p = 1e5;
		FastIO::writesp(m, q, p);
		FastIO::pc('\n');
		for (int i = 1; i <= m;i++){
			FastIO::writesp(dist(myrand), dist(myrand));
			FastIO::pc('\n');
		}
		for (int i = 1; i <= q;i++){
			FastIO::writesp(dist2(myrand), dist2(myrand));
			FastIO::pc('\n');
		}
		FastIO::flush();
		return 0;
