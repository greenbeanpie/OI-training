...
namespace P3413
{
	...
	int dfs(int pos, bool lim, bool first, int pre1, int pre2)
	{
		if (!pos)
		{
			return 1;
		...
		int ans = 0;
		if (!lim && !first && dp[pos][pre1 + 1][pre2 + 1] != -1)
		{
			return dp[pos][pre1 + 1][pre2 + 1];
		}
		...
		for (int i = 0; i <= up; i++)
		{
			if (i != pre1 && i != pre2)
			{
				ans += dfs(pos - 1, lim && i == up, first && !i, (!first || i) ? i : -1, pre1);
				ans%=p;
			}
		}
		if (!lim && !first)
		{
			dp[pos][pre1 + 1][pre2 + 1] = ans%p;
		}
		return ans%p;
	}
	...
	int main()
	{
		string a, b;
		FastIO::read(a, b);
		FastIO::writeln(solve(b) - solve(a - 1));
		return 0;
	}
};
