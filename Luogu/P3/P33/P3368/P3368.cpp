...
struct SegTree
{
	...
	{ // cl:current left(当前的左范围)
		...
		lazy_add[p * 2] = (lazy_add[p * 2] * lazy_mul[p] + lazy_add[p]);
		lazy_add[p * 2 + 1] = (lazy_add[p * 2 + 1] * lazy_mul[p] + lazy_add[p]);
		tree[p * 2] = (tree[p * 2] * lazy_mul[p] + lazy_add[p] * (cmid - cl + 1));
		tree[p * 2 + 1] = (tree[p * 2 + 1] * lazy_mul[p] + lazy_add[p] * (cr - cmid));
		lazy_mul[p] = 1;
		lazy_add[p] = 0;
	}
