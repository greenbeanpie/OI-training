...
namespace Main
{
	...
	uniform_int_distribution<> dist1e5(1, 1e5), dist1e9(1, 1e9), dist20(15, 20);

	int main()
	{
		int n = /* 1e5 */ 1e5, k = dist20(myrand);
		FastIO::writesp(n, k);
		FastIO::pc('\n');
		for (int i = 1; i <= n; i++)
		{
			FastIO::writesp(dist1e9(myrand));
		}
		return 0;
	}
};
...
{
#ifndef ONLINE_JUDGE
	freopen(problemname "25.in", "w", stdout);
#endif
